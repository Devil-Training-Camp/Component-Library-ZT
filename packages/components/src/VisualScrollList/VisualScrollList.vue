...
const fixScrollTop = () => {
	...
	if (state.startOffset > 0 && !isPositive) {
		...
		const newScrollTop = positionDataArr[state.start].endPos - state.startOffset;
...
const updateHeightAndPos = () => {
	...
	state.pillarDomHeight = positionDataArr.length > 0 ? positionDataArr[positionDataArr.length - 1].endPos : 0;
...
<template>
	<div>
		<div ref="listContainer" class="ea-list-container" @scroll="onScroll">
			<!-- 用于显示滚动条 -->
			<div class="ea-pillarDom" :style="{ height: `${state.pillarDomHeight}px` }"></div>
			<!-- 数据列表 -->
			<div ref="listContent" class="ea-list-content" :style="styleTranslate">
				<div v-for="item in renderData" :key="item.id" :data-index="item.arrPos" class="ea-list-item">
					<!-- 通过插槽，自定义列表显示内容 -->
